...
We distinguished between "trial" studies, and those implementing more routinely administered vaccines. Likewise, we distinguished between studies pertaining to the "introduction" of a vaccine in an area, versus those that pertained to costing of a vaccine that was already "routine".
...
The model's coefficient estimates (excluding the smoothed bivariate spatial interpolation estimates, which are too high density to be printed in one table) are below.
...
Of note in the above table: the cost of introductin (as opposed to routine) vaccination campaigns is greater than $2 USD per dose. Human resources is the greatest expense, while both transportation and wastage also contribute significantly to overall cost.
...
We can also use our model to project costs for each item onto every African country. The below projections assume an "introductory" (as opposed to "routine") campaign, with 2015 GDP and in 2015 currency.
...
The below table shows item point estimates on average for all African countries.  
...
According to our model, the average cost per dose (for non-routine immunisation programmes) is on the order of `r round(total, digits = 2)` USD.
...
Full code and data for this report are available at [https://github.com/joebrew/gsk](https://github.com/joebrew/gsk). This work was carried out by Elisa Sicuri and Joe Brew, in collaboration with GSK. A debt of gratitude is owed to Celine Aerts, who helped with data collection.

All analysis was carried out in the R programming language in `r x$R.version$version.string` on a `r x$R.version$system` system. 